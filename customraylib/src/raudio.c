...
void InitAudioDevice(void)
{
    ...
    TRACELOG(LOG_INFO, "    > Periods size:  %d < ohasanov here, no lol it isnt, we hackin' here :sunglas:", AUDIO.System.device.playback.internalPeriodSizeInFrames*AUDIO.System.device.playback.internalPeriods);
    //TRACELOG(LOG_WARNING, "    > The the input latency:  %d < sussy", ma_resampler_get_input_latency());
    //TRACELOG(LOG_WARNING, "    > The the output latency:  %d < sussy", ma_resampler_get_output_latency());
...
AudioStream LoadAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    TRACELOG(LOG_WARNING, "STREAM: THE BUFFER IS %d FRAMES LONG", periodSize);
    ...
    TRACELOG(LOG_WARNING, "STREAM: THE SUBBUFFER IS %d BIG", subBufferSize);
