...
uint32_t _readRGBA4(const void* data){
	...
	uint8_t r = (uint8_t)(((*clr >> 12) & 0b00001111)<< 4);
	uint8_t g = (uint8_t)(((*clr >> 8) & 0b00001111) << 4);
	uint8_t b = (uint8_t)(((*clr >> 4) & 0b00001111) << 4);
	uint8_t a = (uint8_t)(((*clr >> 0) & 0b00001111) << 4);

	return (r | (g << 8) | (b << 16) | (a << 24));
}


uint32_t _readRGB565(const void* data){
	uint16_t  *clr = (uint16_t*)data;

    //image->data[i] = (unsigned short)r << 12 | (unsigned short)g << 8 | (unsigned short)b << 4 | (unsigned short)a;
	uint8_t r = (uint8_t)(((*clr >> 11) & 0b00011111)<< 3);
	uint8_t g = (uint8_t)(((*clr >> 5) & 0b00111111) << 2);
	uint8_t b = (uint8_t)(((*clr >> 0) & 0b00011111) << 3);
	uint8_t a = 255;
...
static inline readFunc _determineReadFunction(int format, uint8_t *bpp){
	switch (format) {
        ...
        case PIXELFORMAT_UNCOMPRESSED_R5G6B5: *bpp = 2; return _readRGB565;
...
{
    printf("\e[1;38;5;236m[INFO] \e[38;5;236mIMAGE: Image Cropping is happening!\n");
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
    ...
    else
    {
        ...
        unsigned char *croppedData = (unsigned char *)malloc((int)(crop.width*crop.height)*bytesPerPixel);
