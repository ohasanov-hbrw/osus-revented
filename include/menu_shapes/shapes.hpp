...
//Create Triangle Strip Vertices for a Trapezoid, x and y is the coordinates of the bottom left corner
std::vector<Vector2> Create_Shape_Trapezoid(enum SHAPES_POSITION_ANCHORS anchor, float widthBottom, float height, float x, float y, float leftWidth, float rightWidth);

//Update already existing object to save on alloc calls 
void Update_Shape_Trapezoid(enum SHAPES_POSITION_ANCHORS anchor, float widthBottom, float height, float x, float y, float leftWidth, float rightWidth, std::vector<Vector2>* points);