...
private:
  ...
  struct LoadedTexture {
    ...
    std::atomic<int> setid{-1};
    float opacity = 0.0f;

    LoadedTexture() = default;

    // Move constructor (allows std::vector::resize)
    ...
    // Explicitly prevent accidental copies
    LoadedTexture(const LoadedTexture &) = delete;
    LoadedTexture &operator=(const LoadedTexture &) = delete;
  };
  std::atomic<bool> loaderShouldStop;
  std::atomic<bool> textureOpsDone;

  std::vector<LoadedTexture> icons;
  std::vector<std::atomic<int>> iconStates;
  MULTITHREAD_THREAD backgroundLoader;
