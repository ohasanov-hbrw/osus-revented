...
RenderTexture2D LoadRenderTextureMipmapped(int width, int height, int mipmaps)
{
    RenderTexture2D target = { 0 };
    ...
    if (target.id > 0)
    {
        ...
        // Create color texture (default to RGBA)
        target.texture.id = rlLoadTexture(NULL, width, height, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, mipmaps);
        target.texture.width = width;
        target.texture.height = height;
        target.texture.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
...
RenderTexture2D LoadRenderTextureMSAA(int width, int height)
{
    RenderTexture2D target = { 0 };
